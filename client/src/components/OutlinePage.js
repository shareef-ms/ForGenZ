...
const TIPS = [
  '💡 Start with a Title, end with Closing for best results',
  '💡 Use Stats slides to make your data stand out',
  '💡 SWOT works great for business and academic topics',
  '💡 Mix chart types to keep your audience engaged',
  '💡 Add a Quote slide to add credibility',
  '💡 Roadmap slides are perfect for project presentations',
];
...
export default function OutlinePage({ nav, topic, style, slideCount, detail }) {
  ...
  async function generateOutline() {
    ...
    setError('');
    try {
      const res = await axios.post('http://localhost:5000/api/outline', { topic, style, slideCount });
  ...
  async function buildDeck() {
    ...
    try {
      ...
      const { title, slides, templateId, fonts, speakerNotes } = res.data;

      // Apply the user-selected theme from topic page
      const THEME_MAP = {
  ...
  function updateHeading(idx, val) {
    setOutline(prev => prev.map((s, i) => i === idx ? { ...s, heading: val } : s));
  }

  function updateType(idx, type) {
    setOutline(prev => prev.map((s, i) => i === idx ? { ...s, type } : s));
    setEditingType(null);
  }

  function addSlide() {
    setOutline(prev => [...prev, { num: prev.length + 1, type: 'bullets', heading: 'New Slide', description: '' }].map((s, i) => ({ ...s, num: i + 1 })));
  }

  function deleteSlide(idx) {
    if (outline.length <= 2) return;
    setOutline(prev => prev.filter((_, i) => i !== idx).map((s, i) => ({ ...s, num: i + 1 })));
  }
  ...
  function shuffle() {
    setOutline(prev => {
      ...
      const middle = [...prev.slice(1, -1)].sort(() => Math.random() - 0.5);
      return [first, ...middle, last].map((s, i) => ({ ...s, num: i + 1 }));
    });
  }
  ...
  const selectedThemeId = localStorage.getItem('forgenz-theme-id') || 'dark-tech';

  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A', color: '#fff', fontFamily: 'Syne, sans-serif', display: 'flex', flexDirection: 'column' }}>
      {/* Nav */}
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 32px', borderBottom: '1px solid #1a1a1a', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={nav.toTopic} style={{ padding: '8px 16px', background: 'transparent', border: '1px solid #333', borderRadius: '8px', color: '#888', fontSize: '13px', cursor: 'pointer', fontFamily: 'Syne, sans-serif' }}>← Back</button>
          <button
            onClick={buildDeck}
            disabled={building || loading || outline.length === 0}
            style={{ padding: '10px 24px', background: building ? '#1a1a1a' : '#D4FF00', color: building ? '#444' : '#000', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '800', cursor: building ? 'not-allowed' : 'pointer', fontFamily: 'Syne, sans-serif' }}
          >
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        ...
        <div style={{ flex: 1, padding: '28px 32px', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px' }}>
            <div>
              <div style={{ fontSize: '22px', fontWeight: '800', letterSpacing: '-0.5px', marginBottom: '4px' }}>Outline · {topic} 📋</div>
              <div style={{ fontSize: '13px', color: '#444', fontFamily: 'DM Sans, sans-serif' }}>
                {outline.length} slides · <span style={{ color: '#555' }}>drag ⠿ to reorder · click type badge to change</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              {/* Theme preview dot */}
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', background: '#111', border: '1px solid #222', borderRadius: '8px' }}>
                <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: TYPE_COLORS[selectedThemeId] || '#D4FF00' }} />
                <span style={{ fontSize: '11px', color: '#555', fontFamily: 'DM Sans, sans-serif' }}>{selectedThemeId}</span>
              </div>
              <button onClick={addSlide} style={{ padding: '8px 14px', background: '#111', border: '1px solid #222', borderRadius: '8px', color: '#888', fontSize: '12px', cursor: 'pointer', fontFamily: 'Syne, sans-serif' }}>+ Add</button>
              <button onClick={shuffle} style={{ padding: '8px 14px', background: '#111', border: '1px solid #222', borderRadius: '8px', color: '#888', fontSize: '12px', cursor: 'pointer', fontFamily: 'Syne, sans-serif' }}>⇄ Shuffle</button>
              <button onClick={generateOutline} style={{ padding: '8px 14px', background: '#111', border: '1px solid #222', borderRadius: '8px', color: '#888', fontSize: '12px', cursor: 'pointer', fontFamily: 'Syne, sans-serif' }}>↺ Regenerate</button>
            </div>
          </div>

          {/* Tip */}
          {!loading && outline.length > 0 && (
            <div style={{ background: '#111', border: '1px solid #1a1a1a', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px', fontSize: '12px', color: '#555', fontFamily: 'DM Sans, sans-serif' }}>
              {tip}
            </div>
          )}
          ...
          {loading && (
            <div style={{ textAlign: 'center', padding: '80px 0' }}>
              ...
              <div style={{ width: '32px', height: '32px', border: '3px solid #1a1a1a', borderTop: '3px solid #D4FF00', borderRadius: '50%', margin: '0 auto 16px', animation: 'spin 0.8s linear infinite' }} />
              <div style={{ fontSize: '15px', color: '#444', fontFamily: 'DM Sans, sans-serif' }}>Generating outline for "{topic}"...</div>
            </div>
          )}

          {/* Error */}
          {error && (
            <div style={{ textAlign: 'center', padding: '40px', background: '#110a0a', border: '1px solid #2a1a1a', borderRadius: '12px' }}>
              <div style={{ fontSize: '14px', color: '#FF6B6B', marginBottom: '12px', fontFamily: 'DM Sans, sans-serif' }}>{error}</div>
              <button onClick={generateOutline} style={{ padding: '10px 24px', background: '#D4FF00', color: '#000', border: 'none', borderRadius: '8px', cursor: 'pointer', fontFamily: 'Syne, sans-serif', fontWeight: '700' }}>Try Again</button>
            </div>
          )}
          ...
          {!loading && outline.map((slide, idx) => (
            ...
            >
              {/* Drag handle */}
              <span style={{ color: '#333', cursor: 'grab', fontSize: '16px', flexShrink: 0, userSelect: 'none' }} title="Drag to reorder">⠿</span>

              {/* Number */}
              <span style={{ fontSize: '12px', color: '#444', minWidth: '22px', fontFamily: 'DM Sans, sans-serif' }}>{String(slide.num).padStart(2, '0')}</span>

              {/* Type badge */}
              <div style={{ position: 'relative', flexShrink: 0 }} ref={editingType === idx ? typeMenuRef : null}>
                <span
                  onClick={() => setEditingType(editingType === idx ? null : idx)}
                  style={{
                    padding: '4px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: '700',
                    background: `${TYPE_COLORS[slide.type] || '#666'}18`,
                    color: TYPE_COLORS[slide.type] || '#666',
                    border: `1px solid ${TYPE_COLORS[slide.type] || '#666'}35`,
                    cursor: 'pointer', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '4px'
                  }}
                >
                  {SLIDE_TYPES.find(t => t.type === slide.type)?.icon} {slide.type} ▾
                </span>
                ...
                {editingType === idx && (
                  <div style={{
                    ...
                    gap: '4px', minWidth: '240px',
                    boxShadow: '0 12px 40px rgba(0,0,0,0.8)', marginTop: '4px'
                  }}>
                    {SLIDE_TYPES.map(opt => (
                      <button key={opt.type} onClick={() => updateType(idx, opt.type)} style={{
                        padding: '7px 10px', background: slide.type === opt.type ? '#D4FF0015' : 'transparent',
                        border: `1px solid ${slide.type === opt.type ? '#D4FF00' : 'transparent'}`,
                        borderRadius: '6px',
                        color: slide.type === opt.type ? '#D4FF00' : '#777',
              ...
              <input
                ...
                style={{
                  ...
                  color: '#fff', fontSize: '14px', fontFamily: 'Syne, sans-serif',
                  fontWeight: '600', outline: 'none', minWidth: 0
                }}
              />

              {/* Delete */}
              <button onClick={() => deleteSlide(idx)} style={{ background: 'transparent', border: 'none', color: '#333', cursor: 'pointer', fontSize: '14px', padding: '2px 6px', flexShrink: 0, transition: 'color 0.15s' }}
          ...
          {/* Add at bottom */}
          {!loading && outline.length > 0 && (
            <button onClick={addSlide} style={{ width: '100%', padding: '12px', background: 'transparent', border: '1px dashed #1a1a1a', borderRadius: '10px', color: '#333', fontSize: '13px', cursor: 'pointer', marginTop: '6px', fontFamily: 'DM Sans, sans-serif', transition: 'all 0.15s' }}
        ...
        <div style={{ width: '220px', borderLeft: '1px solid #1a1a1a', padding: '24px 18px', overflowY: 'auto', flexShrink: 0 }}>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            {outline.length > 0 ? (
              Object.entries(
                outline.reduce((acc, s) => { acc[s.type] = (acc[s.type] || 0) + 1; return acc; }, {})
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {[
                ...
                { label: 'Theme', value: selectedThemeId },
          ...
          {outline.length > 0 && !loading && (
            <button onClick={buildDeck} disabled={building} style={{ width: '100%', padding: '14px', background: '#D4FF00', color: '#000', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '800', cursor: building ? 'not-allowed' : 'pointer', fontFamily: 'Syne, sans-serif', opacity: building ? 0.7 : 1 }}>
              {building ? 'Building...' : 'Build Deck ✦'}
            </button>
          )}
        </div>
