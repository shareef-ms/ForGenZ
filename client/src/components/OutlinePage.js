...
export default function OutlinePage({ nav, topic, style, slideCount, detail }) {
  ...
  async function generateOutline() {
    ...
    setError('');
    try {
      const res = await axios.post(`${API}/api/outline`, { topic, style, slideCount });
  ...
  function addSlide() { setOutline(prev => [...prev, { num: prev.length + 1, type: 'bullets', heading: 'New Slide', description: '' }].map((s, i) => ({ ...s, num: i + 1 }))); }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A', color: '#fff', fontFamily: 'Syne, sans-serif', display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: m ? '12px 16px' : '16px 32px', borderBottom: '1px solid #1a1a1a', position: 'sticky', top: 0, background: '#0A0A0A', zIndex: 10 }}>
        <div style={{ fontSize: m ? '18px' : '20px', fontWeight: '800', cursor: 'pointer' }} onClick={nav.toLanding}>
          For<span style={{ color: '#D4FF00' }}>GenZ</span>
        </div>
        {!m && <div style={{ fontSize: '13px', color: '#555', fontFamily: 'DM Sans, sans-serif' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button onClick={buildDeck} disabled={building || loading || outline.length === 0}
            style={{ padding: m ? '7px 12px' : '10px 24px', background: building ? '#1a1a1a' : '#D4FF00', color: building ? '#444' : '#000', border: 'none', borderRadius: '8px', fontSize: m ? '12px' : '14px', fontWeight: '800', cursor: building ? 'not-allowed' : 'pointer', fontFamily: 'Syne, sans-serif' }}>
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        {/* Left — outline */}
        <div style={{ flex: 1, padding: m ? '16px 14px 100px' : '28px 32px', overflowY: 'auto' }}>
          {/* Header */}
          <div style={{ display: 'flex', alignItems: m ? 'flex-start' : 'center', justifyContent: 'space-between', marginBottom: '16px', flexDirection: m ? 'column' : 'row', gap: m ? '10px' : '0' }}>
            <div>
              ...
              <div style={{ fontSize: '12px', color: '#444', fontFamily: 'DM Sans, sans-serif' }}>
                {outline.length} slides · drag ⠿ to reorder
              </div>
            </div>
          ...
          {!loading && outline.map((slide, idx) => (
            <div key={idx}
              ...
              style={{
                ...
                padding: m ? '10px 10px' : '12px 14px', borderRadius: '10px', marginBottom: '6px',
            ...
            >
              <span style={{ color: '#333', cursor: 'grab', fontSize: '16px', flexShrink: 0, userSelect: 'none' }}>⠿</span>
              ...
              <div style={{ position: 'relative', flexShrink: 0 }} ref={editingType === idx ? typeMenuRef : null}>
                <span onClick={() => setEditingType(editingType === idx ? null : idx)} style={{
                  padding: m ? '3px 7px' : '4px 10px', borderRadius: '6px', fontSize: m ? '10px' : '11px', fontWeight: '700',
                  background: `${TYPE_COLORS[slide.type] || '#666'}18`, color: TYPE_COLORS[slide.type] || '#666',
                ...
                {editingType === idx && (
                  <div style={{
                    ...
                    background: '#111', border: '1px solid #2a2a2a', borderRadius: '12px', padding: '10px',
                    display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px',
                    minWidth: m ? '200px' : '240px', boxShadow: '0 12px 40px rgba(0,0,0,0.9)', marginTop: '4px'
                  }}>
                    {SLIDE_TYPES.map(opt => (
                      <button key={opt.type} onClick={() => updateType(idx, opt.type)} style={{
                        padding: '7px 8px', background: slide.type === opt.type ? '#D4FF0015' : 'transparent',
                        border: `1px solid ${slide.type === opt.type ? '#D4FF00' : 'transparent'}`,
                        borderRadius: '6px', color: slide.type === opt.type ? '#D4FF00' : '#777',
              ...
              <input value={slide.heading} onChange={e => updateHeading(idx, e.target.value)} style={{
                flex: 1, background: 'transparent', border: 'none', color: '#fff',
                fontSize: m ? '13px' : '14px', fontFamily: 'Syne, sans-serif', fontWeight: '600', outline: 'none', minWidth: 0
              }} />
      ...
      {m && !loading && outline.length > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, padding: '14px 16px', background: '#0A0A0A', borderTop: '1px solid #1a1a1a', zIndex: 10 }}>
          <button onClick={buildDeck} disabled={building} style={{ width: '100%', padding: '16px', background: building ? '#111' : '#D4FF00', color: building ? '#444' : '#000', border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: '800', cursor: building ? 'not-allowed' : 'pointer', fontFamily: 'Syne, sans-serif' }}>
            {building ? '⏳ Building your deck...' : 'Build Deck ✦'}
          </button>
        </div>
      )}
    </div>
